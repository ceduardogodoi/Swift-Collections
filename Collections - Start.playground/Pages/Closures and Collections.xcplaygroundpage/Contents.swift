...
// The plus signal needs two numbers to do the add operation, so then the compiler understands that it needs to sum the left variable, 0 in this case, with the value of the right, the value of first position of the array and so on, until the array values ends
...
let dwarversAfterM = arrayOfDwarfArrays.flatMap { array -> [String] in
    return array.filter { dwarf -> Bool in
        dwarf > "M"
    }
}
let sortedDwarves = dwarversAfterM.sorted()

// `flatMap` as a `for` loop - I've leave this one as a bonus challenge!
var dwarversAfterMForLoop: [String] = []
for array in arrayOfDwarfArrays {
    for dwarf in array {
        if dwarf > "M" {
            dwarversAfterMForLoop.append(dwarf)
        }
    }
}
dwarversAfterMForLoop.sort()
